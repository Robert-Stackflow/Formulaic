...
![MoE Abstract View](/img/moe-abstract-view.png)
...
- MoE 模型主要由两个关键部分组成
  ...
  <img src="/img/00_switch_transformer.png" alt="Switch Layer" style="zoom: 67%;" />
  - 稀疏 MoE 层

    <img src="/img/expert-layer-abstract-view.png" alt="Expert Layer Abstract View" style="zoom: 67%;" />
  ...
  - 门控网络或路由（Gating Network）
    ...
    <img src="/img/router-abstract-view.png" alt="Router Abstract View" style="zoom:67%;" />
...
<img src="/img/moe-just-syntactic.png" alt="MoE just syntactic" style="zoom:67%;" />
...
- 混合专家模型的概念最早可以追溯到 1991 年的工作 [Adaptive Mixture of Local Experts](https://www.cs.toronto.edu/~hinton/absps/jjnh91.pdf)
  ...
  <img src="/img/01_moe_layer.png" alt="MoE layer in LSTM" style="zoom: 80%;" />
...
- 标准的 decoder-only Transformer 架构会在层归一化之后使用前馈神经网络
  ...
  <img src="/img/standard-transformer-Dense.png" alt="Standard Transformer Dense" style="zoom: 50%;" />

- 为了重新学习 token 中的复杂关系，FFN 通常会对输入进行扩展，隐藏层维度很大

  <img src="/img/large-ffn-in-transformer.png" alt="Large FFN in Transformer" style="zoom:50%;" />

- 上述前馈神经网络被称为 Dense 模型，因为所有参数（其权重和偏置）都会被激活，没有任何参数被遗漏，所有参数都会用于计算输出

  <img src="/img/Dense-ffn-activate-all-params.png" alt="Dense FFN activate all params" style="zoom:50%;" />

- 相比之下，稀疏模型仅激活总参数的一部分，举个例子，可以将 Dense 模型分割成多个部分（即所谓的专家），对其重新训练，并且在特定时间只激活一部分专家：

  <img src="/img/split Dense to experts.png" alt="Split Dense to experts" style="zoom:50%;" />

- 其核心思想是，每个专家在训练过程中会学习不同的信息，在进行推理时，只会使用那些与特定任务最相关的专家

- 当接收到一个输入时，可以据此选择最适合特定任务的专家：

  <img src="/img/select expert from sparse model.png" alt="Select Expert from sparse model" style="zoom:50%;" />

- 实际上，这里的专家学到的是更细粒度的信息，而非整个领域的知识，因此称之为“专家”是有一定误导性的，如 ST-MoE 论文的统计：

  <img src="/img/st-moe-expert-specialization.png" alt="ST-MoE expert specialization" style="zoom:50%;" />

- Mixtral 的论文展示了每个 token 被分配到的第一个专家，也表明专家们倾向于关注语法而非特定领域

  <img src="/img/mixtral-show-expert-specialization.png" alt="Mixtral show expert specialization" style="zoom: 67%;" />
...
- 在上面的说明中，我们将专家描述为由一个 Dense 模型分割为不同的子模块，但在实际工程中，专家本身通常是完整的前馈神经网络
  ...
  <img src="/img/experts are actually ffn.png" alt="Expert are actually ffn" style="zoom:67%;" />

- 由于存在多个 Decoder Block，因此一般需要经过多个专家的处理

  <img src="/img/input transfer many experts.png" alt="Input transfer many experts" style="zoom:67%;" />

- 由于不同的专家在不同的 token 处理中存在差异，因此会有多条流向路径产生

  <img src="/img/different experts have different paths.png" alt="Different experts have different paths" style="zoom:67%;" />

- Dense 模型与稀疏模型的对比

  <img src="/img/Dense vs. sparse models.png" alt="Dense vs. sparse models" style="zoom:67%;" />
...
- 门控网络的示意图如下：
  ...
  <img src="/img/router-view.png" alt="Router view" style="zoom:67%;" />

- 这样路由和一组专家组成了 MoE 层

  <img src="/img/moe-layer.png" alt="MoE layer" style="zoom:67%;" />
...
- MoE 层也可以分为 Dense 和 sparse 两种，两者都使用路由器来选择专家，但 Sparse MoE 模型只选择少数专家，而 Dense MoE 模型则会选择所有专家，只是可能分布不同，一般来说，MoE 指的是 Sparse MoE 模型
  ...
  <img src="/img/sparse vs. Dense moe.png" alt="Sparse vs. Dense MoE" style="zoom:67%;" />

- 路由的工作原理

  <img src="/img/router-detail.png" alt="Router Detail" style="zoom: 50%;" />
...
- Shazeer 等人的工作进一步探索了更复杂的门控策略，其中最具代表性的是带噪声的 Top-K 门控（Noisy Top-K Gating），该方法在门控网络中引入可学习的高斯噪声，并仅保留得分最高的 $k$ 个专家
  ...
  - 示意图如下：
    ...
    <img src="/img/router-keep-topk.png" alt="Router KeepTopK" style="zoom: 67%;" />
  ...
  - 示意图如下：
    ...
    <img src="/img/router-keep-topk-select.png" alt="Router keepTopK Select" style="zoom:67%;" />
  ...
  - 示意图如下：
    ...
    <img src="/img/router-keep-topk-softmax.png" alt="Router KeepTopK Softmax" style="zoom:67%;" />

- KeepTopK 策略将每个 token 路由到几个选定的专家，这种方法被称为 token 选择，并允许将给定的 token 发送到一个专家（如 top-1）

  <img src="/img/router-keep-top1.png" alt="Router KeepTop1" style="zoom:67%;" />

- 也可以发送到多个专家（如 top-2）

  <img src="/img/router-keep-top2.png" alt="Router KeepTop2" style="zoom:67%;" />
...
- 该损耗的第一个组成部分是指定专家对整个批次中每个 token 的路由值的和，这个结果表示每个专家的重要性分数，该分数表示无论输入如何，某个特定专家被选中的可能性
  ...
  <img src="/img/auxiliary loss-sum.png" alt="Auxiliary Loss Sum" style="zoom: 67%;" />
...
- 如果重要性分数存在很大差异，那么 CV 将会很高
  ...
  <img src="/img/auxiliary-loss-cv-high.png" alt="Auxiliary Loss CV High" style="zoom:67%;" />

- 如果重要性分数差异较小，那么 CV 较低，也正是我们所希望得到的

  <img src="/img/auxiliary-loss-cv-low.png" alt="Auxiliary Loss cv Low" style="zoom:67%;" />

- 利用这个 CV 值，可以在训练过程中更新辅助损失，使其尽可能降低 CV，从而给予每个专家同等的重要性

  <img src="/img/auxiliary-loss-minimize-cv.png" alt="Auxiliary Loss Minimize cv" style="zoom:67%;" />
...
- 当一个专家达到容量时，多余的 token 将被发送给下一个专家：
  ...
  <img src="/img/expert-capacity-principle.png" alt="Expert Capacity Principle" style="zoom:67%;" />

- token 溢出：如果两位专家都已达到其容量，token 将不会被任何专家处理，而是被发送到下一层

  <img src="/img/expert-capacity-token-overflow.png" alt="Expert capacity token overflow" style="zoom:67%;" />

### 参数对比

- 尽管 MoE 模型推理时需要加载更多参数（稀疏参数），但实际激活的参数更少，因为在推理过程中只使用部分专家（活跃参数）

  <img src="/img/active vs. sprase params.png" alt="Active vs. Sparse params" style="zoom: 50%;" />
...
- Mixtral 8x7B 的模型参数计算
  ...
  <img src="/img/mixtral params calculate.png" alt="Mixtral Params Calculate" style="zoom:50%;" />
...
- Mixtral 参数示意
  ...
  <img src="/img/mixtral params calculate result.png" alt="Mixtral Params Calculate Result" style="zoom:50%;" />
...
<img src="/img/02_moe_block.png" alt="MoE Transformer Encoder" style="zoom: 80%;" />
...
<img src="/img/03_switch_layer.png" alt="Switch Transformer Layer" style="zoom: 80%;" />
...
- 模型架构
  ...
  - 容量因子与 token 溢出
    ...
    <img src="/img/capacity factor and token overflow.png" alt="Capacity Factor and token overflow" style="zoom:67%;" />

  - 容量因子与专家容量的计算关系

    <img src="/img/expert-capacity vs. capacity-factor.png" alt="Expert Capacity vs. Capacity factor" style="zoom:67%;" />

  - 简化的辅助损失

    <img src="/img/switch-transformer-al.png" alt="Switch Transformer Auxiliary Loss" style="zoom:67%;" />
  ...
  - 辅助损失的公式表示
    ...
    <img src="/img/switch-transformer-al-math-form.png" alt="Switch Transformer Auxiliary Loss Math Form" style="zoom: 50%;" />
...
- 在稀疏 MoE 中，训练不稳定往往不是来自专家本身，而是来自路由网络：
  - 路由网络通常输出 logits，然后通过 softmax 或 Top-K + softmax 决定专家分配
    ...
    <img src="/img/old-moe-router-calculate.png" alt="Old MoE Router Calculate" style="zoom:50%;" />
...
- ST-MoE 引入了 Router z-loss，来促使路由输出的值更小——路由网络的正则化技术
  ...
  - Router z-loss 定义为：
    ...
    <img src="/img/router-z-loss-router-calculate.png" alt="Router Z-loss" style="zoom:50%;" />
    $$
  ...
  - 该损失会被加到总训练损失中：
    ...
    <img src="/img/st-moe-total-loss.png" alt="ST-MoE total loss" style="zoom:50%;" />
