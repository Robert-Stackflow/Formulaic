...
<img src="/img/llm/gradaccumulation_diag.png" alt="image.png" style="zoom:67%;" />
...
<img src="/img/llm/dp_diagram.png" alt="image.png" style="zoom:67%;" />
...
- 实现步骤
  ...
  <img src="/img/llm/dp_naive.svg" alt="image.png" style="zoom:80%;" />
  - 等所有反向传播全部完成，得到完整梯度
...
<img src="/img/llm/dp_overlap2.svg" alt="image.png" style="zoom:80%;" />
...
- ZeRO 不同阶段的显存占用
  ...
  <img src="/img/llm/zero_memory.svg" alt="zero_memory.svg" style="zoom:67%;" />

#### ZeRO Stage 1：切分优化器状态（Partitioning Optimizer States）

- 基本思想

  <img src="/img/llm/dp_zero1.gif" alt="dp_zero1.gif" style="zoom:67%;" />
...
- 在通信与计算重叠方面，主要有两种策略
  ...
  <img src="/img/llm/dp_zero1_overlap.svg" alt="dp_zero1_overlap.svg" style="zoom:80%;" />
...
- 基本思想
  ...
  <img src="/img/llm/dp_zero2.gif" alt="dp_zero2.gif" style="zoom:67%;" />
...
<img src="/img/llm/dp_zero2_overlap.svg" alt="dp_zero2_overlap.svg" style="zoom:80%;" />
...
- 参数被分布式存储时，如何进行前向 / 反向传播？
  ...
  - 在执行前向传播、按顺序遍历各层时，会按需取回该层所需的参数，并在使用完之后立即将其从显存中释放
    ...
    <img src="/img/llm/dp_zero3_fwd.svg" alt="dp_zero3_fwd.svg" style="zoom:80%;" />

  - 在反向传播中类似，只是流向相反

    <img src="/img/llm/dp_zero3_bwd.svg" alt="dp_zero3_bwd.svg" style="zoom:80%;" />

- 通信开销问题

  <img src="/img/llm/dp_zero3_overlap.svg" alt="dp_zero3_overlap.svg" style="zoom:80%;" />
  - 通信代价分析
...
<img src="/img/llm/tp_diagram.svg" alt="TP diagram" style="zoom:67%;" />
...
- 列并行（Column-wise / Column-linear Sharding）
  ...
  <img src="/img/llm/tp_diagram2.png" alt="image.png" style="zoom: 50%;" />
...
- 行并行（Row-wise / Row-linear Sharding）
  ...
  <img src="/img/llm/tp_diagram3.png" alt="image.png" style="zoom:50%;" />
...
- MLP 层的并行化
  ...
  <img src="/img/llm/tp_diagram4.png" alt="image.png" style="zoom:67%;" />
  - MLP 通常是：
...
- Multi-Head Attention 的并行化
  ...
  <img src="/img/llm/tp_full_diagram.png" alt="image.png" style="zoom:80%;" />
...
<img src="/img/llm/tp_sp_diagram.png" alt=" in forward: f = no-op ; f* = all-reduce ; g = all-gather ; g* = reduce-scatter             in backward: f = all-reduce ; f* = no-op ; g = reduce-scatter ; g* = all-gather            SP region needs full hidden_dim" style="zoom: 50%;" />
...
<img src="/img/llm/tp_sp_diagram_zoomed.png" alt="image.png" style="zoom:50%;" />
...
- 通信与计算重叠问题
  ...
  <img src="/img/llm/tp_sp_overlap.svg" alt="tp_sp_overlap.svg" style="zoom: 80%;" />
...
<img src="/img/llm/ring-attention.gif" alt="ring-attention.gif" style="zoom:67%;" />
...
- 在casual attention 中，Ring Attention 存在严重的负载不均衡
  ...
  <img src="/img/llm/cp_attnmask.svg" alt="cp_attnmask.svg" style="zoom:67%;" />
...
<img src="/img/llm/cp_zigzagmask.svg" alt="cp_zigzagmask.svg" style="zoom:67%;" />
...
- Zig-Zag Ring Attention 的通信与计算重叠
  - all-gather
    ...
    <img src="/img/llm/cp_overlap_allgather.svg" alt="cp_overlap_allgather.svg" style="zoom:80%;" />
  ...
  - all-to-all (Ring)
    ...
    <img src="/img/llm/cp_overlap_all2all.svg" alt="cp_overlap_all2all.svg" style="zoom:80%;" />
...
<img src="/img/llm/pp_afab.svg" alt="image.png" style="zoom:80%;" />
...
<img src="/img/llm/pp_afab2.svg" alt="pp_afab2.svg" style="zoom:80%;" />
...
<img src="/img/llm/pp_1f1b.svg" alt="image.png" style="zoom:80%;" />
...
- 1F1B 中 bubble 对性能的影响
  ...
  <img src="/img/llm/pp_1f1b_scaling.png" alt="Throughput scaling of pipeline parallelism with varying micro-batch sizes" style="zoom:80%;" />
...
<img src="/img/llm/pp_1f1b_interleaved.svg" alt="pp_1f1b_interleaved.svg" style="zoom:80%;" />
...
- 交错流水线的调度策略
  ...
  <img src="/img/llm/pp_llama3.1_schedule.png" alt="pp_llama3.1_schedule.png" style="zoom:80%;" />
...
- 它们在依赖关系上并不对称：
  ...
  <img src="/img/llm/pp_zerobubble_compgraph.png" alt="image.png" style="zoom: 67%;" />
...
- Zero Bubble 的关键思想是：
  ...
  <img src="/img/llm/pp_zerobubble_ppschedule.png" alt="image.png" style="zoom:67%;" />
...
<img src="/img/llm/pp_zerobubble_dualpipe.png" alt="image.png" style="zoom:50%;" />
...
<img src="/img/llm/ep_moe.png" alt="ep_moe.png" style="zoom:67%;" />
...
- EP 为什么要和其他并行方式一起用
  ...
  <img src="/img/llm/ep_schema.png" alt="ep_schema.png" style="zoom:67%;" />
...
<img src="/img/llm/5d_nutshell_tp_sp.svg" alt="TP & SP diagram" style="zoom: 50%;" />
...
<img src="/img/llm/5d_nutshell_cp.svg" alt="CP diagram" style="zoom:50%;" />
...
<img src="/img/llm/5d_nutshell_ep.svg" alt="EP diagram" style="zoom:50%;" />
...
<img src="/img/llm/5d_full.svg" alt="image.png" style="zoom:50%;" />

- 每种策略的内存节省情况

<img src="/img/llm/5Dparallelism_8Bmemoryusage.svg" alt="5Dparallelism_8Bmemoryusage.svg" style="zoom:50%;" />
...
<img src="/img/llm/what_we_learnt_heatmap.svg" alt="image.png" style="zoom: 80%;" />
...
- Naive Attention 的内存灾难
  ...
  <img src="/img/llm/flashattn.png" alt="image.png" style="zoom: 40%;" />
  - 标准 attention 的计算流程是：
...
<img src="/img/llm/flashattn2.png" alt="image.png" style="zoom:40%;" />
...
- 浮点数的表示——用于表示一个浮点数的比特被分成三部分：
  ...
  <img src="/img/llm/sign-mantissa-exponent.svg" alt="sign-mantissa-exponent.svg" style="zoom: 20%;" />
...
- 不同数值的表示范围
  ...
  <img src="/img/llm/mixedprecision.png" alt="image.png" style="zoom: 25%;" />
...
- 不同数值的分辨率（resolution）
  ...
  <img src="/img/llm/mixedprecision_2.png" alt="image.png" style="zoom:25%;" />
...
- 然而，FP8 预训练带来了一个极其严峻的挑战：数值稳定性
  ...
  <img src="/img/llm/fp8-loss.png" alt="fp8-loss" style="zoom: 80%;" />
...
- 首次公开成功的大规模 FP8 混合精度训练来自 DeepSeek-V3 的技术报告
  ...
  <img src="/img/llm/fp8_diagram.png" alt="image.png" style="zoom:60%;" />
...
<img src="/img/llm/conclusion_llama3_parallelism.png" alt="image.png" style="zoom: 67%;" />
