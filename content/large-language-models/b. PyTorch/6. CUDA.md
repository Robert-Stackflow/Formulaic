...
- CPU 与 GPU 的组织架构区别
  ...
  <img src="/img/ArchCPUGPUcores.png" alt="Comparing the relative capabilities of the basic elements of CPU and GPU architectures" style="zoom: 40%;" />
...
- GPU 的计算层次结构：线程（Thread） → Warp → SM
  ...
  <img src="/img/diving_primergpu.svg" alt="image.png" style="zoom: 67%;" />
  - 线程（Thread）
  ...
  - 对于 if / else 分支，不同的线程会走不同的分支，会被串行执行（warp divergence），因此 GPU 极其不适合复杂的控制流
    ...
    <img src="/img/simtVolta.png" alt="Threads in the same warp cannot execute statements in the if-block in parallel with statements in the else-block. The timeline shows that only one group of threads or the other can be active." style="zoom:50%;" />
...
- 内存层次结构
  ...
  <img src="/img/diving_primergpu2.svg" alt="image.png" style="zoom: 67%;" />
...
<img src="/img/T100GPUMemLevels.png" alt="Local to each SM in the GPU is a 256kB register file, a 128kB unified data cache split between L1 and shared memory, and 64kB of constant caches. All SMs share a 64MB L2 cache and 32GB of global memory." style="zoom: 33%;" />
...
- CUDA 的执行流程
  ...
  <img src="/img/CUDABlockStructure.png" alt="CPU invokes a kernel, which is structured as a 6-by-2 grid of blocks on the GPU; CPU can continue while grid executes on the GPU." style="zoom:25%;" />
...
<img src="/img/kernelThreads.png" alt="Each thread works on a different index (threadID) in an array of inputs to produce an array of outputs" style="zoom:50%;" />
...
- CUDA 的执行层级是一个树形结构
  ...
  <img src="/img/CUDAThreadConfiguration.png" alt="Thread Configuration" style="zoom: 25%;" />
...
<img src="/img/CUDAMemoryArchitecture.png" alt="Memory Architecture" style="zoom:20%;" />
...
- CUDA Stream
  ...
  <img src="/img/CUDAStream.png" alt="Streams" style="zoom:60%;" />
...
- Default stream
  ...
  <img src="/img/CUDADefaultStream.png" alt="Streams" style="zoom:60%;" />
...
- 异步内存传输
  ...
  - 异步内存传输的示例如下
    ...
    <img src="/img/CUDAAsyncMemoryTransfer.png" alt="Streams" style="zoom:67%;" />
...
- 主机端同步
  ...
  <img src="/img/HostSideSync.png" alt="Streams" style="zoom:50%;" />
...
- 设备端同步
  ...
  <img src="/img/DeviceSideSync.png" alt="Streams" style="zoom:50%;" />
  - 上述例子的执行流程：
    - 主机创建事件 `end_2` 并初始化
...
- Roofline 模型：模型在一个计算平台的限制下，到底能达到多快的浮点计算速度
  ...
  <img src="/img/roofline.png" alt="img" style="zoom:45%;" />
  - 可以看到，最高算术强度为
...
- 内存访问模式示例
  ...
  <img src="/img/AlignedAndConsecutive.png" alt="Aligned and Consecutive" style="zoom: 25%;" />
  - 对齐但不连续（Aligned but Non-sequential）：内存地址虽然对齐，但线程访问的不是连续地址。在现代 NVIDIA GPU 上，这种模式仍可能被合并为一次操作，但早期计算能力的 GPU 不支持这种合并

  <img src="/img/NotConsecutive.png" alt="Not Consecutive" style="zoom:25%;" />
  - 不连续访问（Not Consecutive）：内存访问既不连续也可能未对齐，需要多次内存事务才能完成

  <img src="/img/Misaligned.png" alt="Misaligned" style="zoom:25%;" />
...
<img src="/img/memorycoalescing.png" alt="image.png" style="zoom: 45%;" />
...
<img src="/img/memorycoalescing4.png" alt="image.png" style="zoom: 50%;" />
...
<img src="/img/tiling.png" alt="image.png" style="zoom:80%;" />
...
<img src="/img/fused_kernels1.png" alt="image.png" style="zoom:67%;" />
...
<img src="/img/fused_kernels2.png" alt="image.png" style="zoom:67%;" />
...
<img src="/img/a0_general.png" alt="image.png" style="zoom:50%;" />
...
<img src="/img/a0_broadcast.png" alt="image.png" style="zoom:50%;" />
...
<img src="/img/a0_reduce_allreduce.png" alt="image.png" style="zoom: 40%;" />
...
<img src="/img/a0_gather_allgather.png" alt="image.png" style="zoom:40%;" />
...
<img src="/img/a0_scatter_reducescatter.png" alt="image.png" style="zoom:40%;" />
...
<img src="/img/a0_reduce_scatter.gif" alt="image.png" style="zoom: 67%;" />

- 下一个动画展示的是 AllGather 阶段，在该阶段结束时，每个 GPU 都获得了 AllReduce 操作的完整结果：

<img src="/img/a0_all_gather.gif" alt="image.png" style="zoom:67%;" />
...
<img src="/img/a0_barrier.png" alt="image.png" style="zoom:50%;" />
