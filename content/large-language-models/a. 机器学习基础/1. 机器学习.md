...
- 在候选假设集合 $H$ 中，找到给定数据 $D$ 时可能性最大的假设 $h$，$h$ 即称为**极大后验假设**（MAP）
  > 其中，$P(D)$ 是不依赖于 $h$ 的常量
- 假设 $H$ 中每个假设有相同的先验概率，则简化为只考虑 $P(D|h)$，称为给定 $h$ 时数据 $D$ 的似然度，而称 $h_{ML}=\arg\max_{h\in H}P(D|h)$ 为**极大似然假设**
...
其中 $H(p)$ 为常量，因此**最小化 KL 散度等价于最小化交叉熵**
...
隐马尔可夫模型（Hidden Markov Model, HMM）是用于描述带有**隐含状态序列**的随机过程的概率模型，其核心在于：
...
![word2vec原理](img\word2vec原理.png)

- Word2Vec 是 Google 于 2013 年提出的词向量学习工具，其核心目标是**将离散的单词映射到连续向量空间**，从而以数学方式度量词语之间的语义关联。通过向量化表示，模型能够在低维空间中捕捉语义关系并降低计算复杂度
...
![cbow_skip-gram](img\cbow_skip-gram.png)
...
然而，直接计算完整 softmax 的训练开销非常大，实际应用中效率较低。为提升训练效率，提出了两种改进方法：**Hierarchical Softmax** 和 **Negative Sampling**
...
以上公式的**计算量大**，因为分母是 V 项求和；而 Negative Sampling 将 softmax 转换为**二分类**：
...
- 卷积核（滤波器）：在 CNN 中，与权重集合相关的是卷积核（kernel）；
  ...
  - 而在卷积神经网络中，隐藏层中的神经元的感受野比较小，只能看到上一层的部分特征，其所能看到的大小即为**卷积核大小**
...
## 7.7 残差网络（Residual Neural Network，ResNet）
...
- 残差块（Residual Block）
  ...
  ![包含以及不包含1x1卷积层的残差块](img\包含以及不包含 1x1 卷积层的残差块.png)
