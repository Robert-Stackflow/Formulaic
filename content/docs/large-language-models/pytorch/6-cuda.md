...
- 参考资料
  - 帖子
    ...
    - [深度学习编译器中的Tiling：多级分块、并行映射与向量化](https://zhuanlan.zhihu.com/p/1980258687571473536)
...
- MFU
  - MFU（Maximum FLOPs Utilization）指的是 GPU 实际算力消耗占理论峰值算力的百分比，它衡量GPU 的“满载”程度：
  ...
  - 高 MFU（>50%-70%）：表示训练很高效，GPU 充分利用

  - 低 MFU (<30%)：可能存在瓶颈，需要优化 batch size、分布式策略、内核调度或混合精度
