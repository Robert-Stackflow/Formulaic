...
- Transformer 模型不直接处理单词的字符串，而是先将文本转换为 token 序列：
...
- 基本思想
  - 与 BPE 或 WordPiece 的 “自底向上合并子词” 不同，Unigram 使用的是自顶向下的剪枝式（pruning）子词学习
...
- 基本思想
  - SentencePiece 不是具体的分词算法，而是一个通用 Tokenizer 框架，内部可使用多种算法，包括 BPE、Unigram、word-level、character-level 或混合策略
...
- 更多关于 `tokenizers` 库的详细方法对比（如 `encode_plus`, `batch_encode_plus`）与参数说明，可参考：[3. tokenizers 库相关参数.md](../l.%20 其他相关框架与知识 / 3.%20tokenizers%20 库相关参数.md)