...
- 单个 Expert 的模型架构复用 ParallelMLP 的基础定义，核心差异在于是否对 Expert 进行TP（Tensor Parallelism，张量并行）切割，具体规则如下：
...
- deepspeed 把 ep 相关的初始化设置放到模型切割之后，megatron 则在模型切割之前一次性把包括ep在内的初始化完成
